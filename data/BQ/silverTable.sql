...
USING 
  (SELECT DISTINCT
    *, 
    ...
    True as is_active
  FROM `retailerdatalakeproject.bronze_dataset.customers`) source
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `retailerdatalakeproject.silver_dataset.orders` target
USING 
  (SELECT DISTINCT
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `retailerdatalakeproject.bronze_dataset.orders`) source
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `retailerdatalakeproject.silver_dataset.order_items` target
USING 
  (SELECT DISTINCT
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `retailerdatalakeproject.bronze_dataset.order_items`) source
...
TRUNCATE TABLE `retailerdatalakeproject.silver_dataset.categories`;

--Step 3: Insert New or Updated Records
INSERT INTO `retailerdatalakeproject.silver_dataset.categories`
SELECT 
  *,
  CASE 
...
FROM `retailerdatalakeproject.bronze_dataset.categories`;

--Step 1: Create the products Table in the Silver Layer
CREATE TABLE IF NOT EXISTS `retailerdatalakeproject.silver_dataset.products`
(
  product_id INT64,
  name STRING,
  category_id INT64,
...
--Step 2: Truncate table
TRUNCATE TABLE `retailerdatalakeproject.silver_dataset.products`;

--Step 3: Insert New or Updated Records
INSERT INTO `retailerdatalakeproject.silver_dataset.products`
SELECT 
  *,
  CASE 
    WHEN category_id IS NULL OR name IS NULL THEN TRUE
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `retailerdatalakeproject.silver_dataset.product_suppliers` target
USING 
  (SELECT 
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `retailerdatalakeproject.bronze_dataset.product_suppliers`) source
...
--Step 2: Truncate table
TRUNCATE TABLE `retailerdatalakeproject.silver_dataset.suppliers`;

--Step 3: Insert New or Updated Records
INSERT INTO `retailerdatalakeproject.silver_dataset.suppliers`
SELECT 
  *,
  CASE 
...
FROM `retailerdatalakeproject.bronze_dataset.suppliers`;
-------------------------------------------------------------------------------------------------------------
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `retailerdatalakeproject.silver_dataset.customer_reviews` target
USING 
  (SELECT 
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `retailerdatalakeproject.bronze_dataset.customer_reviews`) source
